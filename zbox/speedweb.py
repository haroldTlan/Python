...
urls = (
    ...
    '/api/nas_user/(.+)', 'NasUser',
    '/api/nas_group/(.+)', 'NasGroup',
    '/api/samba_config/(.+)', 'SambaConfig',
)
...
class Beep:
    @format()
    def PUT(self):
        return apicxt.force_stop_beep()

class Volume:
    @format()
    def GET(self):
        return error.success([self._as_obj(volume) for volume in adm.Volume.all() if volume.deleted == 0])
    ...
    def _as_obj(self, volume):
        ...
        obj = {'id'                : volume.uuid,
               ...
               'used'              : volume.used,
               'sync'              : volume.syncing,
               'sync_progress'     : volume.sync_progress,
...
class Diagnosis:
    def GET(self):
        ...
        _, o = execute('bash ../speedio/collect_diagnosis.sh', False, logging=False)
...
class Sync:
    ...
    def PUT(self):
        try:
            name, command = input('name'), input('command')
            if command == 'start':
                return apicxt.sync_lun(name)
            else:
                return apicxt.stop_sync_lun(name)
        except Exception as e:
            tb.print_exc() 
...
class NasUser:
    @format
    def GET(self):
        return error.success(samba_config.getuserskey())
