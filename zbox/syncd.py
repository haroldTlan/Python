...
class SyncProcess(object):
    ...
    def start(self):
        ...
        if not os.path.exists('%s/start_sync_lun.pyc' % os.getcwd()):
            ...
            cmd = ['python', 'start_sync_lun.pyc', '-s', '%d'%sleep_second, '-w', tpw, '-n', self.volume.name]
            log.info('start sync volume %s(%s)' % (self.volume.name, self.volume.dev_path))
            self.process = sp.Popen(cmd, stdout=sp.PIPE, close_fds=True, cwd='/home/zonion/speedio')
